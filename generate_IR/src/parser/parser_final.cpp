/*
It's only purpose is to facilitate developping and speed up compiling
*/
...
inline bool alpha_(const char& c) {
    return ((c == '_') || ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z'));
}

inline bool alphanum_(const char& c) {
    return ((c == '_') || ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') ||
            ('0' <= c && c <= '9'));
}

bool isIdentifier(std::string* text) {
    if (!alpha_((*text)[0])) return false;
    for (auto& c : *text) {
...
void Parser::errorLog(const std::string& error) {
    if (currentToken) {
        ...
        throw std::runtime_error(
            error + "\n" +
            tokenizer->fileContent.substr(
                currentToken->pos.index - currentToken->pos.chr,
...
Node Parser::parseFunctionCall() {
    ...
    if (text != ")") errorLog("Expected ')' after function call");
...
Node Parser::parseBlock() {
    if (text.empty() || (text != "{")) errorLog("expected '{'");
